...
import { Clock, Calendar, CheckCircle2, AlertTriangle, UserX, LogIn, LogOut } from 'lucide-react'
      ...
      if (list && list.length > 0) {
        ...
        api.getDailyAttendance().then((records) => {
          ...
          if (rList && rList.length > 0) {
            ...
            const todayRec = rList.find((r: any) => r.employee === user?.id || r.employee_id === user?.id) || rList[0]
            if (todayRec) {
              if (todayRec.check_in) {
                setClockedIn(true)
                setCheckInTime(formatTime(todayRec.check_in))
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <input
                  ...
                  style={{ padding: '8px 14px', background: '#F7F6F3', border: '1px solid #E6E6E6', fontSize: 14, outline: 'none' }}
                />
              </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
            ...
            <tbody>
              {myHistory.map((h, i) => {
                ...
                const hrs = (mins / 60).toFixed(1)
                return (
                  <tr key={h.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>

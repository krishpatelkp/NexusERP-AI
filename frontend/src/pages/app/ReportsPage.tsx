...
import { Users, Clock, CalendarDays, Wallet, Package, CreditCard, ArrowRight, AlertCircle, FileText } from 'lucide-react'
...
export default function ReportsPage() {
  ...
  const renderStatusBadge = (status?: string) => {
    ...
    if (st === 'PENDING' || st === 'PROBATION') {
      return (
        <span style={{ padding: '3px 8px', background: '#FEF7E0', color: '#B06000', fontSize: 11, fontWeight: 700 }}>
          {status || 'Pending'}
        </span>
      )
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            ...
            >
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  ...
                  {isSelected && (
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', background: '#000000', color: '#ffffff' }}>
                      ACTIVE
                    </span>
                  )}
                </div>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 28, borderRadius: 0 }}>
        ...
        {loading ? (
          <div style={{ padding: 40, textAlign: 'center', color: '#767676' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            {selectedReport === 'employee' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {(reportData?.list || []).map((emp: any, i: number) => (
            ...
            {selectedReport === 'attendance' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {(reportData?.list || []).map((r: any, i: number) => (
                    <tr key={r.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>
            ...
            {selectedReport === 'leave' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {(reportData?.list || []).map((l: any, i: number) => (
            ...
            {selectedReport === 'payroll' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {(reportData?.list || []).map((p: any, i: number) => (
                    <tr key={p.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>
            ...
            {selectedReport === 'inventory' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {(reportData?.list || []).map((a: any, i: number) => (
                    <tr key={a.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>
            ...
            {selectedReport === 'payment' && (
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {(reportData?.list || []).map((pm: any, i: number) => (
                    <tr key={pm.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>

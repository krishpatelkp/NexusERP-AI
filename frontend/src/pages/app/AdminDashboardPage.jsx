...
export default function AdminDashboardPage() {
    const [empSummary, setEmpSummary] = useState({ total_employees: 105, active_employees: 102 });
    ...
    const [leaveSummary, setLeaveSummary] = useState({ pending: 3 });
    const [payrollSummary, setPayrollSummary] = useState({ total_net: 85000 });
    ...
    const loadData = async () => {
        try {
            ...
            if (empSum)
                setEmpSummary(empSum);
            if (attDash)
                setAttDashboard(attDash);
            if (leaveSum)
                setLeaveSummary(leaveSum);
            if (paySum)
                setPayrollSummary(paySum);
            if (empList)
                setEmployees(empList);
            if (leaveReqs) {
                const reqList = Array.isArray(leaveReqs) ? leaveReqs : leaveReqs.results || [];
                setPendingLeaves(reqList.filter((l) => {
    ...
    const renderStatusBadge = (status) => {
        ...
        if (st === 'active') {
            return (<span style={{ padding: '3px 8px', background: '#000000', color: '#ffffff', fontSize: 11, fontWeight: 700, borderRadius: 0 }}>
          Active
        </span>);
        }
        if (st === 'probation') {
            return (<span style={{ padding: '3px 8px', background: '#F1F3F4', color: '#3C4043', fontSize: 11, fontWeight: 700, borderRadius: 0 }}>
          Probation
        </span>);
        }
        return (<span style={{ padding: '3px 8px', background: '#FCE8E6', color: '#C5221F', fontSize: 11, fontWeight: 700, borderRadius: 0 }}>
        {status || 'Resigned'}
      </span>);
    };
        ...
        <div style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24, borderRadius: 0 }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
              ...
              <tbody>
                {employees.slice(0, 10).map((emp, i) => (<tr key={emp.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24, borderRadius: 0 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
            ...
            return (<tr key={leave.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>
                    <td style={{ padding: '12px 12px', fontWeight: 600, color: '#000000' }}>

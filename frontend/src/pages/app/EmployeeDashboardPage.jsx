...
import { LogIn, LogOut, CheckCircle2, ChevronDown, ChevronUp } from 'lucide-react';
    ...
    const loadEmployeeData = async () => {
        try {
            ...
            if (attList && attList.length > 0) {
                const todayRec = attList.find((r) => r.employee === user?.id || r.employee_id === user?.id) || attList[0];
                if (todayRec) {
            ...
            if (balList && balList.length > 0) {
                const totalAvail = balList.reduce((sum, b) => sum + (Number(b.remaining_days ?? b.total_allocated ?? 12) || 0), 0);
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
        ...
        <div style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24, borderRadius: 0 }}>
          ...
          <div style={{ fontSize: 24, fontWeight: 700, color: '#000000', lineHeight: 1.2, marginBottom: 8 }}>
            {checkInTime ? (<span style={{ color: '#137333' }}>In: {checkInTime}</span>) : (<span style={{ color: '#767676' }}>Not Marked</span>)}
          </div>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24, borderRadius: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setIsFormOpen(!isFormOpen)} style={{
        ...
        {isFormOpen && (<div style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid #E6E6E6' }}>
            ...
            <form onSubmit={handleLeaveSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                ...
                <select value={leaveForm.leave_type} onChange={(e) => setLeaveForm({ ...leaveForm, leave_type: Number(e.target.value) })} style={{ width: '100%', padding: '10px 14px', background: '#F7F6F3', border: '1px solid #E6E6E6', fontSize: 14, outline: 'none' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                <div>
                  ...
                  <input type="date" required value={leaveForm.start_date} onChange={(e) => setLeaveForm({ ...leaveForm, start_date: e.target.value })} style={{ width: '100%', padding: '10px 14px', background: '#F7F6F3', border: '1px solid #E6E6E6', fontSize: 14, outline: 'none' }}/>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: 13, fontWeight: 600, marginBottom: 6, color: '#000' }}>
                    End Date
                  </label>
                  <input type="date" required value={leaveForm.end_date} onChange={(e) => setLeaveForm({ ...leaveForm, end_date: e.target.value })} style={{ width: '100%', padding: '10px 14px', background: '#F7F6F3', border: '1px solid #E6E6E6', fontSize: 14, outline: 'none' }}/>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea required rows={3} value={leaveForm.reason} onChange={(e) => setLeaveForm({ ...leaveForm, reason: e.target.value })} placeholder="State brief reason for your leave request..." style={{ width: '100%', padding: '10px 14px', background: '#F7F6F3', border: '1px solid #E6E6E6', fontSize: 14, outline: 'none', fontFamily: 'var(--font-body)' }}/>
              </div>

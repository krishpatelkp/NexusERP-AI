...
export default function LeavePage() {
  ...
  const handleApprove = async (id: number) => {
    setAllRequests((prev) =>
      prev.map((r) =>
        r.id === id ? { ...r, status: 'APPROVED', approval_status: 'Approved', leave_status: 'Approved' } : r
      )
    )
    setMyRequests((prev) =>
      prev.map((r) =>
  ...
  const handleReject = async (id: number) => {
    setAllRequests((prev) =>
      prev.map((r) =>
        r.id === id ? { ...r, status: 'REJECTED', approval_status: 'Rejected', leave_status: 'Rejected' } : r
      )
    )
    setMyRequests((prev) =>
      prev.map((r) =>
          ...
          <div style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24 }}>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 13 }}>
                ...
                <tbody>
                  {balances.map((b, i) => (
                    <tr key={b.id || i} style={{ borderBottom: '1px solid #E6E6E6' }}>
                      <td style={{ padding: '14px 12px', fontWeight: 700, color: '#000000' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 20 }}>
        {balances.length > 0 ? (
          balances.map((b: any, idx: number) => (
            <div key={b.id || idx} style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24 }}>
          ...
          [
            ...
            <div key={c.type} style={{ background: '#ffffff', border: '1px solid #E6E6E6', padding: 24 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#767676', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
                {c.type}
              </div>
              <div style={{ fontSize: 32, fontWeight: 700, color: '#000000', lineHeight: 1, marginBottom: 8, letterSpacing: '-0.02em' }}>
        ...
        <form onSubmit={handleApply} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm({ ...form, leave_type: Number(e.target.value) })}
          ...
          <div>
            ...
            <textarea
              ...
              style={{ width: '100%', padding: '10px 14px', background: '#F7F6F3', border: '1px solid #E6E6E6', fontSize: 14, outline: 'none', fontFamily: 'inherit' }}
            />
          </div>

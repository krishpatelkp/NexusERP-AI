...
import { Bot, Send, ArrowLeft, Plus, MessageSquare, Terminal, Sparkles } from 'lucide-react';
export default function AICopilotPage() {
    ...
    const handleSend = async (promptText) => {
        ...
        try {
            ...
            const aiMsg = {
                ...
                tools_used: res.used_tools || res.tools_used || [],
            };
    ...
    return (<div style={{ height: '100vh', display: 'flex', background: '#000000', color: '#ffffff', fontFamily: 'var(--font-body)', overflow: 'hidden' }}>
      ...
      <aside style={{
        ...
        <div style={{ flex: 1, padding: 12, display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' }}>
          ...
          {conversations.map((c) => (<button key={c.id} onClick={() => setActiveConvId(c.id)} style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: '#000000' }}>
        ...
        <div style={{ flex: 1, padding: 32, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {messages.length === 0 && (<div style={{ maxWidth: 640, margin: '40px auto', textAlign: 'center' }}>
              <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, textAlign: 'left' }}>
                ...
                {suggestedPrompts.map((prompt) => (<button key={prompt} onClick={() => handleSend(prompt)} style={{
                ...
                }} onMouseEnter={(e) => (e.currentTarget.style.background = '#222222')} onMouseLeave={(e) => (e.currentTarget.style.background = '#111111')}>
                    <span>"{prompt}"</span>
          ...
          {messages.map((m) => (<div key={m.id} style={{
              ...
              <div style={{
                ...
                {m.role === 'assistant' && m.tools_used && m.tools_used.length > 0 && (<div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid #E6E6E6', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                    <Terminal size={12} color="#767676"/>
                    <span style={{ fontSize: 11, fontWeight: 700, color: '#767676', textTransform: 'uppercase' }}>Used:</span>
                    {m.tools_used.map((t) => (<span key={t} style={{ padding: '2px 6px', background: '#F1F3F4', color: '#3C4043', fontSize: 10, fontWeight: 700, fontFamily: 'monospace' }}>
                        {t}
                      </span>))}
                  </div>)}
              </div>
        ...
        <div style={{ padding: 24, borderTop: '1px solid rgba(255,255,255,0.1)', background: '#0a0a0a' }}>
          <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} style={{ display: 'flex', gap: 12, maxWidth: 900, margin: '0 auto' }}>
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder={isAdmin ? 'Ask Admin query (e.g. "What is today\'s attendance?")' : 'Ask Employee query (e.g. "What is my leave balance?")'} style={{

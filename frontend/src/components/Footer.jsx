...
export default function Footer() {
    return (<footer style={{ background: '#000', color: '#fff', padding: '80px 0 40px', borderTop: '1px solid #1a1a1a' }}>
          ...
          {footerLinks.map((col) => (<div key={col.title}>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
                {col.links.map((link) => (<li key={link}>
                    <a href="#" style={{ fontSize: 13, color: '#888', textDecoration: 'none', transition: 'color 0.2s ease' }} onMouseEnter={(e) => (e.currentTarget.style.color = '#fff')} onMouseLeave={(e) => (e.currentTarget.style.color = '#888')}>
                      {link}
                    </a>
                  </li>))}
              </ul>
